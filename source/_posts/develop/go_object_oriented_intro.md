...
---
# 前言
梳理总结学习Go语言的笔记。关于Go语言中的编程模型与传统的OO模型的知识梳理，包括：OOP基本概念，UML类图以及在Go中如何进行类OO编程。添加关于类型查询、类型断言内容的梳理。
...
## 抽象
- Go使用接口对问题（行为）进行抽象，接口是一组方法（行为）的签名，结构体类型只需实现该组方法即可说明该结构体实现了这个接口（在运行时通过枚举对比方法签名，判断是否赋值给接口变量的结构体实例类型实现了该接口的方法）；
...
- 需要注意的是，Go中没有函数重载以及结构体显式的方法重载；但是，当结构体与被嵌套结构体存在同名方法时，直接调用会覆盖被嵌套结构体的方法。
...
## 开放递归
传统的OOP模型中，都有一个隐藏的`this`指针，在Go中利用方法的接收者进行代替。接收者分为值类型与指针类型；
...
- 2019/07/01：修改部分名词及解释，添加[开放递归](http://landcareweb.com/questions/14659/shi-yao-shi-kai-fang-di-gui)部分的内容
